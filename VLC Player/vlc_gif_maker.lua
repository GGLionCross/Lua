...
default_command = 'ffmpeg -ss {start_timestamp} -to {stop_timestamp} -i "{input_file}" -vf "setpts={pts}*PTS,fps={fps},scale=498:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop {loop} "{output_path}/{output_filename}.gif"'
command = false
output_path = false

-- ** CUSTOM DEBUG TOOLS START ** --
local function log_to_file(message)
    local log_file_path = vlc.config.configdir() .. "/gif_maker.log"
    local log_file = io.open(log_file_path, "a") -- Append to log
    if log_file then
        log_file:write(message .. "\n")
        log_file:close()
    else
        vlc.msg.err("Failed to open log file")
    end
end
...
function gui_create_main_dialog(command, output_path)
    ...
    dlg:add_button("Open Log", open_log, 7, 7)
...
function generate_gif()
    ...
    local pts = 1 / tonumber(speed:get_text())
    ...
    if result == 0 then
        ...
        vlc.osd.message("GIF failed to generate. See log.")
        local timestamp = os.date("%Y-%m-%d %H:%M:%S")
        log_to_file("[" .. timestamp .. "] GIF failed to generate. Code: " .. result .. ", Command: " .. command)
    end
end
